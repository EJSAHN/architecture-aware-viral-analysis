...
def run_pipeline(
    ...
    component_concordance, component_edges, component_group_summary = component_neighbor_concordance(
        registry_df,
        config,
        full_clr_matrix=all_clr,
        component_completeness_df=component_completeness,
    )
    ...
    component_tension_sensitivity_summary = pd.DataFrame()
    component_tension_sensitivity_tests = pd.DataFrame()
    component_tension_correlations = pd.DataFrame()
    component_tension_sensitivity_correlations = pd.DataFrame()
    if not component_tension_scores.empty:
        full_tension_scores = component_tension_scores.loc[
            component_tension_scores.get("full_expected_component_set", False).astype(bool)
        ].copy()
        if not full_tension_scores.empty:
            component_tension_group_summary = group_value_summary(
                full_tension_scores,
                value_col="component_configuration_tension_index",
                group_col="source_group",
                panel_name="componentized_full_sets",
        ...
        component_tension_sensitivity_summary = group_value_summary(
            component_tension_scores,
            value_col="component_configuration_tension_index",
            group_col="source_group",
            panel_name="componentized_all_informative",
            label_name="component_configuration_tension_index",
            n_bootstrap=int(config["statistics"]["bootstrap_iterations"]),
            random_seed=int(config["analysis"]["random_seed"]),
        )
        component_tension_sensitivity_tests = pairwise_value_group_tests(
            component_tension_scores,
            value_col="component_configuration_tension_index",
            group_col="source_group",
            panel_name="componentized_all_informative",
    ...
    if not component_tension_scores.empty and not component_concordance.empty:
        concordance_cols = [
            ...
            for col in [
                "source_group",
                "isolate",
                "neighbor_discordance_score",
                "normalized_neighbor_discordance_score",
                "target_agreement_fraction",
                "unique_target_isolates",
                "full_expected_component_set",
            ]
            if col in component_concordance.columns
        ]
        merge_concordance = component_concordance[concordance_cols].copy()
        # Avoid duplicate completeness fields already present in tension scores.
        merge_concordance = merge_concordance.drop(
            columns=[c for c in ["full_expected_component_set"] if c in component_tension_scores.columns],
            errors="ignore",
        )
        component_tension_scores = component_tension_scores.merge(
            merge_concordance, on=["source_group", "isolate"], how="left"
        )

        full_scores = component_tension_scores.loc[
            component_tension_scores.get("full_expected_component_set", False).astype(bool)
        ].copy()
        if "normalized_neighbor_discordance_score" in full_scores.columns and not full_scores.empty:
            component_tension_correlations = correlation_test_summary(
                full_scores["component_configuration_tension_index"],
                full_scores["normalized_neighbor_discordance_score"],
                panel_name="componentized_full_sets",
                label_name="tension_vs_normalized_neighbor_discordance",
                x_name="component_configuration_tension_index",
                y_name="normalized_neighbor_discordance_score",
                n_bootstrap=int(config["statistics"]["bootstrap_iterations"]),
                random_seed=int(config["analysis"]["random_seed"]),
                method="spearman",
            )

        if "normalized_neighbor_discordance_score" in component_tension_scores.columns:
            component_tension_sensitivity_correlations = correlation_test_summary(
                component_tension_scores["component_configuration_tension_index"],
                component_tension_scores["normalized_neighbor_discordance_score"],
                panel_name="componentized_all_informative",
                label_name="tension_vs_normalized_neighbor_discordance",
                x_name="component_configuration_tension_index",
                y_name="normalized_neighbor_discordance_score",
    ...
    stats_sheets = {
        ...
        "component_tension_sensitivity_summary": component_tension_sensitivity_summary,
        "component_tension_sensitivity_tests": component_tension_sensitivity_tests,
        "component_tension_correlations": component_tension_correlations,
        "component_tension_sensitivity_correlations": component_tension_sensitivity_correlations,
    }
