...
def _component_completeness_lookup(component_completeness_df: pd.DataFrame | None) -> dict[tuple[str, str], dict[str, Any]]:
    if component_completeness_df is None or component_completeness_df.empty:
        return {}
    if not {"source_group", "isolate_key"}.issubset(component_completeness_df.columns):
        return {}
    ...
    for _, row in component_completeness_df.iterrows():
        ...
        lookup[(source_group, isolate_key)] = {
            ...
            "full_expected_component_set": bool(row.get("all_expected_components_present", False)),
        }
    return lookup
...
def component_configuration_tension(
    ...
    for source_group, group_df in component_df.groupby("source_group"):
        ...
        if source_scores.empty:
            continue
        full_scores = source_scores[source_scores["full_expected_component_set"]].copy()
        tension_values = source_scores["component_configuration_tension_index"].to_numpy(dtype=float)
        full_values = full_scores["component_configuration_tension_index"].to_numpy(dtype=float)
        summary_rows.append(
            {
                "source_group": source_group,
                "n_informative_isolates_scored": int(source_scores.shape[0]),
                "n_full_set_isolates_scored": int(full_scores.shape[0]),
                "mean_tension_all_informative": float(np.nanmean(tension_values)),
                "median_tension_all_informative": float(np.nanmedian(tension_values)),
                "sd_tension_all_informative": float(np.nanstd(tension_values, ddof=1)) if source_scores.shape[0] > 1 else 0.0,
                "mean_tension_full_sets": float(np.nanmean(full_values)) if full_values.size else float("nan"),
                "median_tension_full_sets": float(np.nanmedian(full_values)) if full_values.size else float("nan"),
                "sd_tension_full_sets": (
                    float(np.nanstd(full_values, ddof=1)) if full_values.size > 1 else (0.0 if full_values.size == 1 else float("nan"))
                ),
                "median_directional_tension_full_sets": (
                    float(np.nanmedian(full_scores["directional_tension"])) if not full_scores.empty else float("nan")
                ),
                "median_spread_score_full_sets": (
                    float(np.nanmedian(full_scores["spread_score"])) if not full_scores.empty else float("nan")
                ),
                "median_vector_length_cv_full_sets": (
                    float(np.nanmedian(full_scores["vector_length_cv"])) if not full_scores.empty else float("nan")
                ),
