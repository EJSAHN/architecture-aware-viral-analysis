...
def component_neighbor_concordance(df: pd.DataFrame, config: dict[str, Any], full_clr_matrix: np.ndarray | None = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for source_group, group_df in component_df.groupby("source_group"):
        ...
        edge_df = pd.DataFrame([row for row in edge_rows if row["source_group"] == source_group])
        if edge_df.empty:
            continue
        ...
        for isolate, isolate_edges in edge_df.groupby("isolate"):
            ...
            agreement_fraction = top_count / components_present if components_present > 0 else np.nan
            discordance_rows.append(
                {
                    ...
                    "neighbor_discordance_score": 1.0 - agreement_fraction,
                }
            )

        completeness_subset = component_df.loc[component_df["source_group"] == source_group, ["isolate", "normalized_component"]].dropna().drop_duplicates()
        summary_rows.append(
            {
                "source_group": source_group,
                "n_component_records_used": int(len(group_df)),
                "n_isolates_scored": int(edge_df["isolate"].nunique()),
                "n_components_considered": int(edge_df["component_label"].nunique()),
                "median_neighbor_discordance_score": float(
                    pd.DataFrame([row for row in discordance_rows if row["source_group"] == source_group])["neighbor_discordance_score"].median()
                ),
            }
        )
    ...
    if not concordance_df.empty:
        concordance_df = concordance_df.sort_values(
            ["source_group", "neighbor_discordance_score", "components_present"],
            ascending=[True, False, False],
            ignore_index=True,
        )
        concordance_df["discordance_rank_within_group"] = (
            concordance_df.groupby("source_group")["neighbor_discordance_score"].rank(method="dense", ascending=False)
        )
